...
Column {
    ...
    Rectangle {
        id: currenttimestamp
        width: Math.max(15, (timetext.width) + 12)
        height: timetext.height + 12
        ...
        TextEdit {
            id: timetext
            width: Math.min(implicitWidth, currenttimestamp.__maxWidth)
            ...
            Component.onCompleted: {
                if (timetext.hasOwnProperty('linkHovered'))
                    timetext.linkHovered.connect(function() { })
            }
    ...
    Rectangle {
        ...
        color: (model.status === ConversationModel.Error) ? "#ffdcc4" : ( model.isOutgoing ? "light blue" : "light green" )
        Behavior on color { ColorAnimation { } }
